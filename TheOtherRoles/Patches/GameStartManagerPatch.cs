            ...
            public static void Postfix(GameStartManager __instance) {
                ...
                if (AmongUsClient.Instance.AmHost) {
                    ...
                    foreach (InnerNet.ClientData client in AmongUsClient.Instance.allClients.ToArray()) {
                        ...
                        if (dummyComponent != null && dummyComponent.enabled)
                            ...
                            blockStart = true;
                            message += $"<color=#FF0000FF>{client.Character.Data.PlayerName}没有安装更多职业或持有不同版本\n</color>";
                        } else {
                            PlayerVersion PV = playerVersions[client.Id];
                            int diff = TheOtherRolesPlugin.Version.CompareTo(PV.version);
                            if (diff > 0) {
                                message += $"<color=#FF0000FF>{client.Character.Data.PlayerName}的更多职业版本过低 (v{playerVersions[client.Id].version.ToString()})\n</color>";
                                blockStart = true;
                            } else if (diff < 0) {
                                message += $"<color=#FF0000FF>{client.Character.Data.PlayerName}的更多职业版本过高 (v{playerVersions[client.Id].version.ToString()})\n</color>";
                                blockStart = true;
                            } else if (!PV.GuidMatches()) { // version presumably matches, check if Guid matches
                                message += $"<color=#FF0000FF>{client.Character.Data.PlayerName}安装的是经过修改的更多职业（是不是错误安装了GM版？） v{playerVersions[client.Id].version.ToString()} <size=30%>({PV.guid.ToString()})</size>\n</color>";
                                blockStart = true;
                            }
                        }
                    }
                ...
                if (!AmongUsClient.Instance.AmHost) {
                    if (!playerVersions.ContainsKey(AmongUsClient.Instance.HostId) || TheOtherRolesPlugin.Version.CompareTo(playerVersions[AmongUsClient.Instance.HostId].version) != 0) {
                        ...
                        __instance.GameStartText.text = $"<color=#FF0000FF>主持人没有安装更多职业或持有不同版本\n你将在{Math.Round(10 - kickingTimer)}秒之后被踢出</color>";
                        __instance.GameStartText.transform.localPosition = __instance.StartButton.transform.localPosition + Vector3.up * 2;
                    } else {
                        __instance.GameStartText.transform.localPosition = __instance.StartButton.transform.localPosition;
