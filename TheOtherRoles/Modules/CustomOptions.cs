...
namespace TheOtherRoles {
    public class CustomOption {
        ...
        public CustomOption(int id, string name,  System.Object[] selections, System.Object defaultValue, CustomOption parent, bool isHeader) {
            ...
            selection = 0;
            if (id != 0) {
                entry = TheOtherRolesPlugin.Instance.Config.Bind($"预设{preset}", id.ToString(), defaultSelection);
        ...
        public static CustomOption Create(int id, string name, bool defaultValue, CustomOption parent = null, bool isHeader = false) {
            return new CustomOption(id, name, new string[]{"否", "是"}, defaultValue ? "是" : "否", parent, isHeader);
        }
        ...
        public static void switchPreset(int newPreset) {
            ...
            foreach (CustomOption option in CustomOption.options) {
                ...
                option.entry = TheOtherRolesPlugin.Instance.Config.Bind($"预设{preset}", option.id.ToString(), option.defaultSelection);
    ...
    class GameOptionsMenuStartPatch {
        public static void Postfix(GameOptionsMenu __instance) {
            ...
            var commonTasksOption = __instance.Children.FirstOrDefault(x => x.name == "公共任务数").TryCast<NumberOption>();
            if(commonTasksOption != null) commonTasksOption.ValidRange = new FloatRange(0f, 4f);

            var shortTasksOption = __instance.Children.FirstOrDefault(x => x.name == "短任务数").TryCast<NumberOption>();
            if(shortTasksOption != null) shortTasksOption.ValidRange = new FloatRange(0f, 23f);

            var longTasksOption = __instance.Children.FirstOrDefault(x => x.name == "长任务数").TryCast<NumberOption>();
            ...
            foreach (CustomOption option in CustomOption.options) {
                if (option.parent == null) {
                    if (option == CustomOptionHolder.crewmateRolesCountMin) {
                        var optionName = CustomOptionHolder.cs(new Color(204f / 255f, 204f / 255f, 0, 1f), "船员个数");
                        ...
                        var optionName = CustomOptionHolder.cs(new Color(204f / 255f, 204f / 255f, 0, 1f), "中立个数");
                        ...
                        var optionName = CustomOptionHolder.cs(new Color(204f / 255f, 204f / 255f, 0, 1f), "伪装者数");
