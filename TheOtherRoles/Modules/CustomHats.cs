...
namespace TheOtherRoles.Modules {
    ...
    public class CustomHats { 
        ...
        private static class HatManagerPatch {
            static void Prefix(HatManager __instance) {
                ...
                try {
                    ...
                    if (!LOADED)
                        System.Console.WriteLine("导入自定义帽子失败\n" + e);
                }
                LOADED = true;
            }
            ...
            static void Postfix(HatParent __instance, string hatId, int color) {
                if (DestroyableSingleton<TutorialManager>.InstanceExists) {
                    try {
                        ...
                        System.Console.WriteLine("创建帽子失败\n" + e);
                    }
                }
            }     
        }
        ...
        public class HatsTabOnEnablePatch {
            public static string innerslothPackageName = "Innersloth帽子";
        ...
        private static async Task LaunchHatFetcherAsync() {
            try {
                ...
                if (status != HttpStatusCode.OK)
                    System.Console.WriteLine("自定义帽子导入失败\n");
            } catch (System.Exception e) {
                System.Console.WriteLine("未找到自定义帽子\n" + e.Message);
            }
            running = false;
        }
